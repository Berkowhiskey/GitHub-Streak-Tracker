...
public class SvgBadgeService : ISvgBadgeService
{
    ...
    /// <summary>Turkce ay kisaltmalari sunucunun yerel ayarindan bagimsiz olmalidir.</summary>
    private static readonly CultureInfo TurkishCulture = new("tr-TR");

    public string GenerateStreakBadge(BadgeData data, BadgeTheme theme)
    {
        ...
        var lastCommit = data.LastCommitDate is { } date
            ? Escape(date.ToString("d MMM", TurkishCulture))
            : "—";

        return $"""
            <svg xmlns="http://www.w3.org/2000/svg" width="{Width}" height="{Height}" viewBox="0 0 {Width} {Height}" role="img" aria-label="{username} icin {data.CurrentStreak} gunluk streak">
              <title>{username} - {data.CurrentStreak} gunluk streak (rekor: {data.LongestStreak})</title>
              <defs>
              ...
              <text x="103" y="90" font-family="{FontStack}" font-size="12" fill="{palette.MutedText}">gunluk seri</text>
              ...
              <text x="270" y="62" font-family="{FontStack}" font-size="11" letter-spacing="0.5" fill="{palette.MutedText}">REKOR</text>
              <text x="378" y="62" font-family="{FontStack}" font-size="14" font-weight="600" text-anchor="end" fill="{palette.PrimaryText}">{data.LongestStreak} gun</text>

              <text x="270" y="86" font-family="{FontStack}" font-size="11" letter-spacing="0.5" fill="{palette.MutedText}">SON COMMIT</text>
    ...
    public string GenerateNotFoundBadge(string username, BadgeTheme theme)
    {
        var palette = BadgePalette.For(theme);
        var safeUsername = Escape(username);

        return $"""
            <svg xmlns="http://www.w3.org/2000/svg" width="{Width}" height="{Height}" viewBox="0 0 {Width} {Height}" role="img" aria-label="Kullanici bulunamadi">
              <title>{safeUsername} StreakTracker'da kayitli degil</title>
              <rect x="0.5" y="0.5" width="{Width - 1}" height="{Height - 1}" rx="10" fill="{palette.Background}" stroke="{palette.Border}"/>
              ...
              <text x="102" y="58" font-family="{FontStack}" font-size="17" font-weight="600" fill="{palette.PrimaryText}">Kullanici bulunamadi</text>
              <text x="102" y="80" font-family="{FontStack}" font-size="12" fill="{palette.MutedText}">@{safeUsername} StreakTracker'a kayitli degil</text>
            </svg>
            """;
    }

    public string ComputeETag(BadgeData data, BadgeTheme theme)
    {
